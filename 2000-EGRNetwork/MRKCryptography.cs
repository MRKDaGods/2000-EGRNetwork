...
namespace MRK {
    public class MRKCryptography {
        ...
        */
        ...
        public static byte[] SaltedChecksum(byte[] data) {
            using MD5 md5 = MD5.Create();
            //apply salt

            byte[] saltData = Encoding.ASCII.GetBytes(ms_CookedSalt);
            byte[] saltedData = new byte[data.Length + saltData.Length];
            data.CopyTo(saltedData, 0);
            saltData.CopyTo(saltedData, data.Length);

            return md5.ComputeHash(saltedData);
        }
